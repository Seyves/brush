...
var drawStore = []OutcomingMessage{}
...
func websocketHandler(w http.ResponseWriter, r *http.Request) {
    ...
    defer func () {
        ...
        newDrawsStore := []OutcomingMessage{}

        for _, draw := range drawStore {
            if draw.Initiator == nameParam {continue}
            
            newDrawsStore = append(newDrawsStore, draw)
        }

        drawStore = newDrawsStore
    ...
    for {
        ...
        if parsedMessage.Cmd == "line" || parsedMessage.Cmd == "end-line" {
            drawStore = append(drawStore, outcoming)
        }
